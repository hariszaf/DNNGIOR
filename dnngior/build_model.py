...
def _replace_suffixes(ex_id):
    # Replace _e0 with _co first to avoid conflict with _e replacement
    ex_id = re.sub(r'_e0$', '_co', ex_id)
    # Replace _e with _c
    ex_id = re.sub(r'_e$', '_c', ex_id)
    return ex_id


def _check_metabolite_exists(model, metabolite_id):
    return metabolite_id in [met.id for met in model.metabolites]


# [TODO] Add exchange reactions for all medium compounds and for the _e0 compounds present in the model
def refine_model(gpfilledModel, draftModel=None, scale = 1000, unscalled = None):
    ...
    # Add exchange reactions for all medium compounds and exchnage metabolites of the draft model
    # for ex_id in draftModel.medium:
    #     ex = draftModel.reactions.get_by_id(ex_id)
    #     if ex not in mc.medium and _check_metabolite_exists(mc, _replace_suffixes(ex_id)[3:]) :
    #         mc.exchanges.append(ex)
    mc.exchanges.extend(
        ex for ex_id in draftModel.medium
        if (
            ex := draftModel.reactions.get_by_id(ex_id)) not in mc.medium
            and
            _check_metabolite_exists(mc, _replace_suffixes(ex_id)[3:]
        )
    )

    # for ex in draftModel.exchanges:
    #     if ex not in mc.exchanges:
    #         mc.exchanges.append(ex)
    mc.exchanges.extend(ex for ex in draftModel.exchanges if ex not in mc.exchanges)
